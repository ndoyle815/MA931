...
% IS
Sigma(4*para.n+1,2*para.n+1) = (1 - para.hosp_rates(1))*para.da(1)*para.n*para.epsilon;
Sigma(4*para.n+2,2*para.n+2) = (1 - para.hosp_rates(2))*para.da(2)*para.n*para.epsilon;
Sigma(4*para.n+3,2*para.n+3) = (1 - para.hosp_rates(3))*para.da(3)*para.n*para.epsilon;
Sigma(4*para.n+1,4*para.n+1) = -para.gamma;
Sigma(4*para.n+2,4*para.n+2) = -para.gamma;
Sigma(4*para.n+3,4*para.n+3) = -para.gamma;

% IPH
Sigma(5*para.n+1,2*para.n+1) = para.hosp_rates(1)*para.da(1)*para.n*para.epsilon;
Sigma(5*para.n+2,2*para.n+2) = para.hosp_rates(2)*para.da(2)*para.n*para.epsilon;
Sigma(5*para.n+3,2*para.n+3) = para.hosp_rates(3)*para.da(3)*para.n*para.epsilon;
Sigma(5*para.n+1,5*para.n+1) = -para.zeta;
Sigma(5*para.n+2,5*para.n+2) = -para.zeta;
Sigma(5*para.n+3,5*para.n+3) = -para.zeta;

% IH
Sigma(6*para.n+1,5*para.n+1) = para.zeta;
Sigma(6*para.n+2,5*para.n+2) = para.zeta;
Sigma(6*para.n+3,5*para.n+3) = para.zeta;
