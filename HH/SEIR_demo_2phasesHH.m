...
[t, pop] = ode45(@diff_SEIR_model, [0:1:para.maxtime], [ICs.S ICs.E1 ICs.E2 ICs.E3 ICs.IA ICs.IS ICs.IPH ICs.IH ICs.R ICs.Cases ICs.Hosp], opts, para);
...
Classes = struct('S',pop(:,1:para.n),'E1',pop(:,para.n+1:2*para.n),'E2',pop(:,2*para.n+1:3*para.n), ...
            ...
            'IPH',pop(:,6*para.n+1:7*para.n),'IH',pop(:,7*para.n+1:8*para.n),'R',pop(:,8*para.n+1:9*para.n), ...
            'Cases',pop(:,9*para.n+1:10*para.n),'Hosp',pop(:,10*para.n+1:11*para.n),'SD',SD,'t',t);
...
R  = pop(8*para.n+1 : 9*para.n);
Cases = pop(9*para.n+1 : 10*para.n);
Hosp = pop(10*para.n+1 : 11*para.n);
...
% ODE equations
dS = -factor.*S.*(para.beta*(para.tau.*IA + IS + IPH + para.rho.*IH))./para.N + para.omega.*R;
...
dR  = para.gamma.*(IS) + para.delta.*IH - para.omega.*R;
dCases = 3*para.sigma.*para.da.*E3;
dHosp = para.epsilon.*IPH;

dPop = [dS; dE1; dE2; dE3; dIA; dIS; dIPH; dIH; dR; dCases; dHosp];
